...
img = Image.open(r'../../src/nyoba_nopal/0.jpg')
img1 = Image.open(r'../../src/nyoba_nopal/mobilmerah.jpg')
...
def cara2(m1, m2, row, col):
    ...
    for i in range(0,row,3):
        for j in range(0,col,3):
            histh = cv2.calcHist(rgbToHSV(m1[i][j][0],m1[i][j][1],m1[i][j][2]),[0],None,[180],[0,180])
            hists = cv2.calcHist(rgbToHSV(m1[i][j][0],m1[i][j][1],m1[i][j][2]),[1],None,[100],[0,100])
            histv = cv2.calcHist(rgbToHSV(m1[i][j][0],m1[i][j][1],m1[i][j][2]),[2],None,[100],[0,100])
            print(histh)
            print(hists)
            print(histv)
            # l = rgbToHistogram(m1[i][j][0],m1[i][j][1],m1[i][j][2])
